...
def evaluate_message_expression(msg: T, expression: str, context: Dict[str, Any] = None) -> Any:
...
class Telemetry:
    ...
    def register_topic(self, name: str, message_type: str, configuration: Topic) -> None:
        """
    ...
    def evaluate_measurement_field(self, field: MeasurementField, msg: T, configuration: Topic, context: Dict[str, Any] = None) -> WriteField:
        value = evaluate_message_expression(msg, field.value, context)
        tags = {}

        for tag, tag_expression in field.tags.items():
            tag_value = evaluate_message_expression(
                msg,
                tag_expression,
                context
            )
    ...
    def evaluate_measurement(self, measurement: Measurement, msg: T, configuration: Topic) -> List[WriteField]:
        fields = []

        for field in measurement.measurement_fields:
            try:
                if field.for_each is not None:
                    for idx, each in enumerate(evaluate_message_expression(msg, field.for_each)):
                        fields.append(
    ...
    def evaluate_measurements(self, msg: T, configuration: Topic) -> List[WriteMeasurement]:
        measurements = []
        ...
        for measurement in configuration.measurements:
            try:
                measurements.append({
                    ...
                    "fields": self.evaluate_measurement(
                        measurement, msg, configuration
                    )
                })
            except EvaluationError as e:
